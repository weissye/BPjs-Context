...
public class ContextStorageModificationStrategy implements StorageModificationStrategy {
  @Override
  public StorageConsolidationResult.Success incomingModifications(StorageConsolidationResult.Success success, BProgramSyncSnapshot bProgramSyncSnapshot, Set<BThreadSyncSnapshot> set) {
    Map<String, MapProxy.Modification<Object>> updates = success.getUpdates();
    if (updates.isEmpty() || updates.keySet().stream().noneMatch(k->k.startsWith("CTX.Entity: "))) return success;
    Map<String, Object> currentMap = bProgramSyncSnapshot.getDataStore();
